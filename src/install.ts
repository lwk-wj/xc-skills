...
export async function installSkills(options: InstallOptions) {
  const { sourceDir, targetAgents, selectedSkills, scope, method, strategy } = options
  const s = p.spinner()
  ...
  for (const agent of targetAgents) {
    ...
    if (scope === 'project') {
      // 项目模式：从 Agent 的默认路径中提取隐藏文件夹名 (例如从 ~/.trae/skills 提取出 .trae)
      // 假设 agent.path 格式为 ~/.xxx/skills
    ...
    try {
      ...
      s.stop(`成功同步到: ${agent.name} (${scope} 模式)`)
    } catch (err: any) {
